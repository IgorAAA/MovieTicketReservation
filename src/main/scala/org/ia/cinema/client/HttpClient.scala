...
class ImdbHttpClient[F[_]: ConcurrentEffect, Out](implicit ec: ExecutionContext)
  ...
  override def response(reqUri: Uri)(implicit dec: Decoder[Out]): F[Out] =
    for {
      ...
      resp <- clientBuilder.resource
        .use { client =>
          client.expect[Out](reqUri)
        }
        .handleErrorWith(
          err => logger.error(err.getMessage) *> Sync[F].raiseError(err)
        )
      _ = logger.debug(resp.toString)
    } yield resp
}

class ImdbHttpStreamClient[F[_]: ConcurrentEffect](
  ...
  def retriveImdbMovieTitle[A: Decoder](reqUri: Uri): Stream[F, A] =
    for {
      ...
      resp <- client
        ...
        .flatMap(
          _.body.chunks.through(byteParserC(SingleValue)).through(decoder[F, A])
        )
        .handleErrorWith(
          err =>
            Stream.eval(logger.error(err.getMessage)) >> Stream.raiseError(err)
        )
    } yield resp
}
