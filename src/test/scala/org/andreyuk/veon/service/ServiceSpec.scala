  ...
  "CinemaService" should {
    "register a movie" in {
      val res = for {
        ...
        _ <- service.registerMovie(
          RegisterMovieMessage(imdbId, screenId, seatsAvailable)
        )
    ...
    "register a movie twice" in {
      val res = for {
        ...
        _ <- service.registerMovie(
          RegisterMovieMessage(imdbId, screenId, SeatsAvailable(100))
        )
        movie <- service.retrieveMovieInfo(RetrieveMovieInfo(imdbId, screenId))
      } yield movie
    ...
    "register a movie and try to retrieve non-existent movie" in {
      val res = for {
        ...
        movie <- service.retrieveMovieInfo(
          RetrieveMovieInfo(imdbId, ScreenId("NONEXISTENT"))
        )
      } yield movie
...
object ServiceSpec {
  ...
  val testHttpClient: HttpClient[IO, ImdbResponse] =
    new HttpClient[IO, ImdbResponse] {
      override def response(reqUri: Uri)(implicit dec: Decoder[ImdbResponse]): IO[ImdbResponse] = IO.pure(ImdbResponse(TestUtils.movieTitle))
    }
}
